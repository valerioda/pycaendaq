...
def main():
    ...
    arg("-st", "--software_trigger", action=st, help="software_trigger")
    ...
    if args["software_trigger"]:
        trig_source = "SwTrg"
        software_rate = 1000 # Hz
        print(f"Setting software trigger with rate {software_rate} Hz")
    else:
        trig_source = "TrgIn"
        print(f"Setting trigger from TrigIn")
    ...
    with device.connect(dig2_uri) as dig:
        ...
        for n in range(n_iter + 1):
            ...
            if save_temperature:
                temp_names = ["tempsensfirstadc","tempsenshottestadc",
                              "tempsenslastadc","tempsensairin",
                              "tempsensairout","tempsenscore","tempsensdcdc"]
